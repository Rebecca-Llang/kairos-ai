...
from database.operations import (
    init_db, add_chat_message, add_memory, get_chat_history, get_all_memories, get_database_stats
)
...
class MigrationConfig:
    ...
    def _read_json_file(self, file_path: Path, expected_type: type = list) -> Optional[Any]:
        ...
        try:
            ...
            if not isinstance(data, expected_type):
                print(f"❌ {file_path.name} is not a valid JSON {expected_type.__name__}")
                return None
    ...
    def verify_migration(self) -> bool:
        ...
        try:
            ...
            if memories:
                print(f"  Sample memory: {memories[0]['memory_key']} = {memories[0]['memory_value'][:30]}...")
